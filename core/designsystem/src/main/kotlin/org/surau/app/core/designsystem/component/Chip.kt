    ...
    FilterChip(
        ...
        border = FilterChipDefaults.filterChipBorder(
            ...
            disabledBorderColor = MaterialTheme.colorScheme.onBackground.copy(
                alpha = SurauChipDefaults.DISABLED_CHIP_CONTENT_ALPHA,
            ),
            disabledSelectedBorderColor = MaterialTheme.colorScheme.onBackground.copy(
                alpha = SurauChipDefaults.DISABLED_CHIP_CONTENT_ALPHA,
            ),
            selectedBorderWidth = SurauChipDefaults.ChipBorderWidth,
        ),
        colors = FilterChipDefaults.filterChipColors(
            ...
            disabledContainerColor = if (selected) {
                MaterialTheme.colorScheme.onBackground.copy(
                    alpha = SurauChipDefaults.DISABLED_CHIP_CONTAINER_ALPHA,
                )
            } else {
                Color.Transparent
            },
...
fun ChipPreview() {
    SurauTheme {
        SurauBackground(modifier = Modifier.size(80.dp, 20.dp)) {
            SurauFilterChip(selected = true, onSelectedChange = {}) {
                Text("Chip")
            }
        }
    }
