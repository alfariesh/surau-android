...
/**
 * Authenticated user/account endpoints, served by the auth client (Bearer + token refresh).
 */
interface SurauUserApi {

    @GET("auth/introspect")
    suspend fun introspect(): IntrospectDto

    @GET("user/profile")
    suspend fun profile(): UserAccountDto

    @PATCH("user/onboarding")
    suspend fun completeOnboarding(@Body body: OnboardingRequestDto): UserAccountDto
